...
static const unsigned int PRE_BLOSSOM_POW_TARGET_SPACING = 30;
static const unsigned int POST_BLOSSOM_POW_TARGET_SPACING = 30;
static_assert(PRE_BLOSSOM_POW_TARGET_SPACING >= POST_BLOSSOM_POW_TARGET_SPACING, "Blossom target spacing must be less or equal than pre-Blossom target spacing.");
...
struct Params {
    ...
    int nMinerLicenseActivationHeight;
    int GetFirstEnforcedMinerLicenseBlockHeight() const {
		return nMinerLicenseActivationHeight;
	};
