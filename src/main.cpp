...
#if defined(NDEBUG)
# error "Vectorium cannot be compiled without assertions."
#endif
        ...
        bool (*isInitBlockDownload)(const CChainParams&))
{
	...
	if (tx.IsCoinBase()) {
		...
		BOOST_FOREACH(const CTxOut& out, tx.vout) {
			...
			if (nHeight >= chainparams.GetConsensus().GetFirstEnforcedMinerLicenseBlockHeight() && ExtractDestination(out.scriptPubKey,out_dest) && !chainparams.IsLicensedMiner(keyIO.EncodeDestination(out_dest)))
    ...
    if (overwinterActive) {
        ...
        if (IsExpiredTx(tx, nHeight)) {
            ...
            return state.DoS(
                    expiredDosLevel,
                    error("ContextualCheckTransaction(): transaction is expired. Resending when caught up with the blockchain, or manually setting the vectd txexpirydelta parameter may help."),
...
{
    CAmount nSubsidy = 1 * COIN; // default

    if (nHeight >= 1 && nHeight <= 15)
    {
        return 1000000 * COIN; // first 15 blocks are premine, 15M in total
    }

    if (nHeight >= 35000000) 
    {
        return 0; // end of rewarding after block 35000000 ... some 33 years after genesis
    }
