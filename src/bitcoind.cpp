...
bool AppInit(int argc, char* argv[])
{
    ...
    if (mapArgs.count("-?") || mapArgs.count("-h") ||  mapArgs.count("-help") || mapArgs.count("-version"))
    {
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  vectd [options]                     " + _("Start Zcash Daemon") + "\n";
    ...
    try
    {
        ...
        } catch (const missing_vect_conf& e) {
            auto confFilename = GetArg("-conf", BITCOIN_CONF_FILENAME);
            fprintf(stderr,
                (_("Before starting vectd, you need to create a configuration file:\n"
                   "%s\n"
                   "It can be completely empty! That indicates you are happy with the default\n"
                   "configuration of vectd. But requiring a configuration file to start ensures\n"
                   "that vectd won't accidentally compromise your privacy if there was a default\n"
        ...
        if (fCommandLine)
        {
            fprintf(stderr, "Error: There is no RPC client functionality in vectd. Use the vect-cli utility instead.\n");
            exit(EXIT_FAILURE);
        }
#ifndef WIN32
        ...
        if (fDaemon)
        {
            fprintf(stdout, "Vectorium server starting\n");
