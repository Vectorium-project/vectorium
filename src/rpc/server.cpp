...
UniValue stop(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop Vectorium server.");
    // Event loop will exit after current HTTP requests have been handled, so
    // this reply will get back to the client.
    StartShutdown();
    return "Vectorium server stopping";
}
...
{
    return "> vect-cli " + methodname + " " + args + "\n";
}
...
std::string experimentalDisabledHelpMsg(const std::string& rpc, const std::vector<string>& enableArgs)
{
    ...
    return "\nWARNING: " + rpc + " is disabled.\n" +
        "To enable it, restart vectd with the following command line options:\n"
        + cmd + "\n\n" +
        "Alternatively add these two lines to the vect.conf file:\n\n"
        + config;
}
