...
//            "  \"coinbasevalue\" : n,               (numeric) maximum allowable input to coinbase transaction, including the generation award and transaction fees (in Satoshis)\n"
...
#else
        throw JSONRPCError(RPC_METHOD_NOT_FOUND, "vectd compiled without wallet and -mineraddress not set");
#endif
    }
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Vectorium is not connected!");

    if (IsInitialBlockDownload(Params()))
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Vectorium is downloading blocks...");
