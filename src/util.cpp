...
const char * const BITCOIN_CONF_FILENAME = "vect.conf";
const char * const BITCOIN_PID_FILENAME = "vectd.pid";
...
#else
    const char* pszModule = "Vectorium";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\vect
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\vect
    // Mac: ~/Library/Application Support/vect
    // Unix: ~/.vect
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "vect";
#else
    fs::path pathRet;
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it)
    {
        ...
        // Don't overwrite existing settings so command line settings override vect.conf
...
{
    return "\n" +
           FormatParagraph(strprintf(_("In order to ensure you are adequately protecting your privacy when using Vectorium, please see <%s>."),
...
std::string LicenseInfo()
{
    return "\n" +
           ...
           FormatParagraph(strprintf(_("Copyright (C) 2019-%i The Vectorium Developers"), COPYRIGHT_YEAR)) + "\n" +
